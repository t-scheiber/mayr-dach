...
Coolify configuration: Dockerfile build pack, `/Dockerfile.demo`, exposed port `8080`, domain `https://mayr-dach.thomasscheiber.com`, health check `/de/`. The root redirects to `/de/`. Both locale prefixes are explicit for static hosting. A proxied Cloudflare CNAME points to the Proxmox tunnel; tunnel traffic reaches the Coolify proxy at `https://192.168.1.200`.
